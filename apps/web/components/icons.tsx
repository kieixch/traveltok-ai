...
/** Brand mark: gradient rounded square with a plane. */
export function Logo({ size = 40, className = "" }: { size?: number; className?: string }) {
  return (
    <span
      className={`inline-flex shrink-0 items-center justify-center rounded-2xl bg-gradient-to-br from-emerald-400 via-teal-400 to-cyan-500 text-white shadow-lg shadow-emerald-500/30 ${className}`}
      style={{ width: size, height: size }}
    >
      <PlaneIcon style={{ width: size * 0.55, height: size * 0.55 }} />
    </span>
  );
}
